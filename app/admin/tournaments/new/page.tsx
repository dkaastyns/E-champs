...
export default function NewTournamentPage() {
	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		toast.promise(createTournamentMutation.mutateAsync(payload), {
		...
		<PageTransition className="max-w-2xl">
			<div className="mb-8">
				<Link
					...
					className="text-[#6520EE] hover:underline text-sm mb-2 inline-block"
				>
			...
			<RevealOnScroll delay={100}>
				<form
					...
					className="space-y-4 bg-[#0d0d0d] border border-[#1a1a1a] p-8"
				>
					<div className="grid grid-cols-2 gap-4">
						<div>
							...
							<input
								name="name"
								required
								className="w-full bg-[#1a1a1a] border border-[#1a1a1a] text-white px-4 py-2 focus:border-[#6520EE] focus:outline-none"
							/>
						</div>
						<div>
							<label className="block text-sm text-gray-400 mb-1">
					...
					<div className="grid grid-cols-3 gap-4">
						...
						<div>
							<label className="block text-sm text-gray-400 mb-1">
								Fee (Rp) <span className="text-red-500">*</span>
							</label>
							<input
